//
//  UILabel+BKAttributeTextTapAction.m
//
//  Created by chendb on 17/3/11.
//  Copyright © 2016年 facebac. All rights reserved.
//

#import "UILabel+BKAttributeTextTapAction.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>

@implementation BKAttributeModel

@end


@implementation UILabel (BKAttributeTextTapAction)
...
- (id<BKAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<BKAttributeTapActionDelegate>)delegate
{
...
- (void)yb_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <BKAttributeTapActionDelegate> )delegate
{
    [self yb_getRangesWithStrings:strings];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                BKAttributeModel *model = self.attributeStrings[j];
...
- (void)yb_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            BKAttributeModel *model = [BKAttributeModel new];
